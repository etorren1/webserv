...
void Client::makePostResponse(char **envp)
{
	...
	if (cgiDone == false)	// флаг cgi записан == false
	{
		...
		// 			reader.seekg(totalBytesRead);
		// 		}
		// 		else
		// 		{
		// 			totalBytesRead -= (bytesRead - wr);
		// 			reader.seekg(totalBytesRead);
		// 		}
		// 	}
		// 	if (wr == 0) {
		...
		// 		}
		// 		printf("\n");
		// 		std::cout << "reader size: " << getStreamSize() << "\n";
		// 		exit(1);
		// 	}
	...
	if (cgiDone == true)											//если все данные передались в cgi
	{
...
void Client::passThroughCgi()
{
	...
	while (1)
	{
		...
		bytesRead = reader.gcount();
		// totalBytesRead += bytesRead;
		wrtRet = write(pipe1[PIPE_OUT], buff, bytesRead);

		if (wrtTotal + wrtRet + 1 >= reader_size) //
			status++;
